    ...
    def purchase_product(
        ...
        with transaction.atomic():
            with connection.cursor() as cursor:
                ...
                total_price -= self.calculate_cart_discount(
                    payload, store=store, cursor=cursor
                )
                for item in payload:
                    product = get_object_or_404(
    ...
    def calculate_cart_discount(
        ...
        if (
            ...
            with transaction.atomic():
                with connection.cursor() as cursor:
                    cursor.execute(
                        f"SELECT pg_advisory_xact_lock_shared({store_lock});"
                    )
                    store = get_object_or_404(Store, pk=store_id)
                    return self.calculate_cart_discount(
                        purchase_products, store=store, cursor=cursor
                    )
        ...
        if cursor is not None and store is not None:
            ...
            with transaction.atomic():
                with connection.cursor() as cursor:
                    ...
                    store = get_object_or_404(Store, pk=store_id)
                    return self.validate_purchase_policy(
                        payload=payload, cursor=cursor, store=store
                    )
