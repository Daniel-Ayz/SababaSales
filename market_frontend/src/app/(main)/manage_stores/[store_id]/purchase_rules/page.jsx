...
export default function PurchasePolicies({ params }) {
  ...
  useEffect(() => {
    const fetchPolicies = async () => {
      ...
      try {
        const response = await axios.post(`${process.env.NEXT_PUBLIC_STORES_ROUTE}${store_id}/get_purchase_policies`, {
          user_id: user.id,
          store_id: store_id
        });
        ...
        for (const policy of response.data) {
          ...
          const combination_functions = await axios.post(`${process.env.NEXT_PUBLIC_STORES_ROUTE}${store_id}/get_combine_function`, {
            store_id: store_id,
            to_discount: false,
            target_id: policy.id
          });
  ...
  const handleAddPolicy = async () => {
    try {
      ...
      const response = await axios.post(`${process.env.NEXT_PUBLIC_STORES_ROUTE}${store_id}/add_purchase_policy`, {
        role: {
          user_id: user.id,
          store_id: store_id,
        },
  ...
  const handleRemovePolicy = async (index) => {
    try {
      ...
      const response = await axios.delete(`${process.env.NEXT_PUBLIC_STORES_ROUTE}${store_id}/remove_purchase_policy`, {
        data: {
          role: {
            user_id: user.id,
            store_id: store_id
